...
struct OsmapTrackingFrame
{
  cv::Mat relative_pose;
  size_t ref_keyframe_id;
  double time;
  bool lost;
};
...
public:
  ...
  */
  ...
  vector<OsmapTrackingFrame*> vectorTrackingFrames;
  ...
  */
  Osmap(System &_system, size_t cam_x,size_t cam_y);
  ...
  */
  ...
  void mapSaveAndShutdown(string basefilename);
  ...
  */
  ...
  void mapLoad(string yamlFilename, bool noSetBad = false, bool pauseThreads = true, bool loadTracking = true);


  /*
  Calls mapLoad and setup the tracker to attempt to track the newest keyframe in the map.
  */

  void mapLoadTracking(string yamlFilename, bool loadTracking = true);
  ...
  /**
   * Save Frame data from tracking, usually "map.tracking".
   * @param filename full name of the file to be created and saved.
   */

  int trackingFramesSave(string filename);

  int trackingFramesLoad(string filename);
  ...
  */
  void serialize(const Mat &k, const float &b, SerializedK *serializedK);

  /**
  Reconstruct the intrinsic camera matrix K from protocol buffer message.
  All 4 fields are required. baseline optional
  ...
  */
  void deserialize(const SerializedK &serializedK, Mat &k, float b);
  ...
  */
  ...
  void deserialize(const SerializedFeature& feature, MapPoint** mappoint, cv::KeyPoint &keypoint, cv::Mat& descriptors, size_t index, float& right, float& depth, float bf);


  // TrackingFrame ====================================================================================================

  void serialize(const OsmapTrackingFrame& frame, SerializedTrackingFrame* serializedFrame);

  int serialize(const vector<OsmapTrackingFrame*>& vectorFrames, SerializedTrackingFramesArray& serializedTrackingFramesArray);

  OsmapTrackingFrame* deserialize(const SerializedTrackingFrame& serializedTrackingFrame);

  int deserialize(const SerializedTrackingFramesArray& serializedTrackingFramesArray);


  void setTrackingDataToTracker();

  void getTrackingDataFromTracker();


void serialize(const KeyFrame::MargEdge &edge, SerializedPoseEdge *serializedEdge);
void deserialize(const SerializedPoseEdge &serializedEdge, KeyFrame::MargEdge &edge);
void serialize(const std::vector<KeyFrame::MargEdge> &edges, SerializedKeyframe &keyframe);
void deserialize(const SerializedKeyframe &keyframe, std::vector<KeyFrame::MargEdge> &edges);
void deserialize_edges(const SerializedKeyframeArray &serializedKeyframeArray, std::vector<OsmapKeyFrame*> &keyframes);
